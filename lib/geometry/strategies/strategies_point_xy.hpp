	...
	namespace strategy
	{
		namespace distance
		{
			...
			*/
			...
			struct pythagoras
			{
				typedef typename select_type_traits<
					             typename support::coordinate<P1>::type,
					             typename support::coordinate<P2>::type
				                 >::type T;
				...
				(distance_result)) operator()(const P1& p1, const P2& p2) const
				{
					// Calculate distance using Pythagoras
					// (Leave comment for Doxygen)
					...
					(impl::compute_pythagoras<P1, P2, support::dimension<P1>::value, T>::result(p1, p2), true);
				}
			};
			...
			*/
			...
			{
				typedef typename select_type_traits<
					             typename support::coordinate<P>::type,
					             typename support::coordinate<PS>::type
				                 >::type T;
				...
				inline distance_result operator()(const P& p, const segment<const PS>& s) const
				{
					BOOST_STATIC_ASSERT((boost::mpl::equal_to<
						support::dimension<P>,
						support::dimension<PS> >::type::value));
					...
					*/
					...
					typename boost::remove_const<PS>::type v, w;
					...
					typename boost::remove_const<PS>::type projected;
			...
			class winding
			{
				...
				public :
					...
					inline bool operator()(const segment<const PS>& s, state_type& state) const
					{
						...
						if (up || down)
						{
							typedef typename support::coordinate<P>::type T;
			...
			class bashein_detmer
			{
				private :
					...
					struct sums
					{
						...
						void centroid(PC& point)
						{
							typedef typename support::coordinate<PC>::type TPC;
							point = sum;
							if (sum_a2 != 0)
							{
								divide_value(point, static_cast<TPC>(3.0 * sum_a2));
							}
							else
							{
								// exception?
		...
		namespace envelope
		{
			...
			struct grow_xy
			{
				struct state
				{
					...
					{
						mbr.min_corner() = b.min_corner();
						mbr.max_corner() = b.max_corner();
					}
				};
