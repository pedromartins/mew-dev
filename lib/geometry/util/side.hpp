...
namespace geometry
{
	...
	inline typename select_type_traits<typename support::coordinate<S>::type,
			typename support::coordinate<P>::type>::type
		point_side(const S& s, const P &p)
	{
		typedef typename select_type_traits<typename support::coordinate<S>::type,
			typename support::coordinate<P>::type>::type T;
