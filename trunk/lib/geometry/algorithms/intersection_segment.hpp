...
namespace geometry
{
	...
	*/
	...
	/*!
		...
		- \a S1 first segment type
		- \a S2 second segment type
	...
	*/
	...
	inline intersection_result intersection_segment(const S1& s1, const S2& s2, O_IT out)
	{
		...
		BOOST_CONCEPT_ASSERT( (ConstPoint<typename S1::point_type>) );
		BOOST_CONCEPT_ASSERT( (ConstPoint<typename S2::point_type>) );
		intersection_result result;

		typedef typename select_type_traits<typename S1::coordinate_type, typename S2::coordinate_type>::type T;
		T dx1 = get<0>(s1.second) - get<0>(s1.first);
		T dx2 = get<0>(s2.second) - get<0>(s2.first);
		T dy1 = get<1>(s1.second) - get<1>(s1.first);
		T dy2 = get<1>(s2.second) - get<1>(s2.first);

		T wx = get<0>(s1.first) - get<0>(s2.first);
		T wy = get<1>(s1.first) - get<1>(s2.first);
        ...
        if(equals(d, 0))
        {
            if(equals(a, 0) && equals(b, 0))
            {
				...
				if (! vertical
						&& (std::min(get<0>(s1.first), get<0>(s1.second)) > std::max(get<0>(s2.first), get<0>(s2.second))
							|| std::min(get<0>(s2.first), get<0>(s2.second)) > std::max(get<0>(s1.first), get<0>(s1.second))))
				{
					...
					return result;
						...
						&& (std::min(get<1>(s1.first), get<1>(s1.second)) > std::max(get<1>(s2.first), get<1>(s2.second))
							|| std::min(get<1>(s2.first), get<1>(s2.second)) > std::max(get<1>(s1.first), get<1>(s1.second))))
				{
				...
				else
				{
					...
					result.ra = vertical
									...
									: wx / dx2;
					result.rb = vertical
									? (get<1>(s1.second) - get<1>(s2.first)) / dy2
									: (get<0>(s1.second) - get<0>(s2.first)) / dx2;
					...
					*out = P(get<0>(s2.first) + result.ra * dx2, get<1>(s2.first) + result.ra * dy2);
					out++;
					...
					*out = P(get<0>(s2.first) + result.rb * dx2, get<1>(s2.first) + result.rb * dy2);
					out++;
					return result;
				}
            }
			...
			&& result.rb >= 0.0 && result.rb <= 1.0)
        {
			...
			*out = P(get<0>(s1.first) + result.ra * dx1, get<1>(s1.first) + result.ra * dy1);
			out++;
