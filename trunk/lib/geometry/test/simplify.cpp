...
void test_simplify_linestring()
{
	...
	// Check using linestring, OBSOLETE
	/*{
	...
	{
		geometry::linestring<P> simplified;
		geometry::simplify(line.begin(), line.end(), std::back_inserter(simplified), 3.0);
		BOOST_CHECK(simplified.size() == 2);
	}

	// define strategy
	typedef geometry::strategy::simplify::douglas_peucker
		<
			typename geometry::linestring<P>::const_iterator,
			std::back_insert_iterator<geometry::linestring<P> >,
			geometry::strategy::distance::xy_point_segment<P>
		>
		DOUGLAS;


	// Check using linestring and strategy, OBSOLETE
	/*{
		geometry::linestring<P> simplified;
		BOOST_CHECK(simplified.size() == 2);
	}*/
	...
	{
		geometry::linestring<P> simplified;
		geometry::simplify(line.begin(), line.end(), std::back_inserter(simplified), 3.0, DOUGLAS());
		BOOST_CHECK(simplified.size() == 2);
	}
}
