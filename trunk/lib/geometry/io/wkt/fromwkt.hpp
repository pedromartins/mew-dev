...
namespace geometry
{
		...
		namespace wkt
		{
			...
			*/
			...
			inline bool parse_container(TOK::iterator& it, TOK::iterator end, O_IT out)
			{
				...
				{
					P point;
					impl::parsing_assigner<P, 0, support::dimension<P>::value>::run(it, end, point);
			...
			*/
			...
			parse_point(const std::string& wkt, P& point)
			{
				...
				impl::parsing_assigner<P, 0, support::dimension<P>::value>::run(it, tokens.end(), point);
				return true;
			}
	...
	{
		line.clear();
		return impl::wkt::parse_linestring<P>(wkt, std::back_inserter(line));
	}
