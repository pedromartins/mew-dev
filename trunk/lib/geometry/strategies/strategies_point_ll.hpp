			...
			class haversine
			{
				public :
					...
					inline distance_result operator()(const P1& p1, const P2& p2) const
					{
						T1 lon1, lat1;
						T2 lon2, lat2;
						to_radian(p1, lon1, lat1);
						to_radian(p2, lon2, lat2);
						return calc(lon1, lat1, lon2, lat2);
					}

				private :
					double m_radius;
					typedef typename support::coordinate<P1>::type T1;
					typedef typename support::coordinate<P2>::type T2;

					inline distance_result calc(const T1& lon1, const T1& lat1, const T2& lon2, const T2& lat2) const
					{
			...
			*/
			...
			class ll_point_segment
			{
				public :
					...
					inline distance_result operator()(const P& p, const segment<const PS>& s) const
					{
						...
						convert_dr(p, pr);
						convert_dr(s.first, ps1);
						convert_dr(s.second, ps2);
						return calc(pr, ps1, ps2);
					}

				private :
					typedef point_ll<radian, typename support::coordinate<P>::type> PR;
					double m_radius;
				...
				public :
					...
					inline bool operator()(const segment<const P>& segment, state_type& state) const
					{
						if (get<0>(segment.first) != get<0>(segment.second))
						{
							typedef point_ll<radian, typename support::coordinate<P>::type> PR;
							PR p1, p2;
							convert_dr(segment.first, p1);
							convert_dr(segment.second, p2);
		...
		namespace envelope
		{
			...
			*/
			...
			struct grow_ll
			{
				...
				struct state
				{
					typedef typename support::coordinate<PB>::type T;
					bool has_west;
					T min_lat, max_lat;
					...
					void envelope(box<PB>& mbr)
					{
						...
						mbr.min_corner().lat(min_lat);
						mbr.max_corner().lat(max_lat);
						if (! has_west)
						{
							mbr.min_corner().lon(min_lon1);
							mbr.max_corner().lon(max_lon1);
						}
						else
						{
							// Get both ranges
							...
							{
								mbr.min_corner().lon(min_lon2);
								mbr.max_corner().lon(max_lon2);
							}
						}
					}
				};
