...
void example_as_wkt_vector()
{
	...
	std::cout << geometry::make_as_wkt(std::make_pair(v.begin(), v.end())) << std::endl;
}
...
void example_point_ll_convert()
{
	...
	geometry::point_ll<geometry::radian, double> rad;
	geometry::convert_dr(deg, rad);
...
void example_intersection_linestring1()
{
	...
	geometry::intersection(cb, line.begin(), line.end(),
...
void example_intersection_linestring2()
{
	...
	VV vector_out;
	geometry::intersection(cb, vector_in.begin(), vector_in.end(),
...
void example_simplify_linestring1()
{
	...
	geometry::from_wkt("linestring(1.1 1.1, 2.5 2.1, 3.1 3.1, 4.9 1.1, 3.1 1.9)", line);
	geometry::simplify(line.begin(), line.end(), std::back_inserter(simplified), 0.5);
	std::cout
...
void example_simplify_linestring2()
{
	...
	L line;
	geometry::from_wkt("linestring(1.1 1.1, 2.5 2.1, 3.1 3.1, 4.9 1.1, 3.1 1.9)", line);
	geometry::strategy::simplify::douglas_peucker<L::const_iterator, OUT, DS> simplification;
...
void example_simplify_linestring3()
{
	...
	geometry::strategy::simplify::douglas_peucker<std::vector<P>::const_iterator, OUT, DS> simplification;
	geometry::simplify(line.begin(), line.end(), OUT(std::cout, "\n"), 0.5, simplification);
}
...
void example_length_linestring()
{
	...
	std::cout << "linestring length is "
		<< length(line_ll.begin(), line_ll.end()) / 1000
		<< " kilometers " << std::endl;
}
...
void example_length_linestring_strategy()
{
	...
	std::cout << "linestring length is "
		<< length(line_ll.begin(), line_ll.end(), strategy::distance::vincenty<LL, LL>() )/(1000)
		<< " kilometers " << std::endl;
}
...
void example_envelope_linestring()
{
	...
	geometry::envelope(line.begin(), line.end(), box);

	std::cout << "box is " << box.min_corner() << "-" << box.max_corner() << std::endl;
}
...
namespace geometry
{
	namespace traits
	{
		...
		template <> struct coordinate<example_point_1> { typedef double type; };
